...
修2 (2026-05-10):
- 同步呼叫 migrate_readme.py 更新該專案的 README.md
- README 自動同步:已實作功能 / Skip / 開發中 / 待實作 四區塊
...
def on_task_completed(task_file: Path, new_status: str = "done") -> dict:
    ...
    Args:
        task_file: Task 檔案路徑（如 $IDEAS2TASKS_TASKS_DIR/working-issue/tasks/T001.md）
    ...
    Returns:
        {
            ...
            "idea_file": "...",
            "idea_synced": True,
            "readme_updated": True,        # 修2: README 是否更新成功
            "readme_error": None,         # 修2: README 更新失敗時的錯誤訊息
        }
    """
    result = {
    ...
    # 4. 更新專案 README.md（同步已實作/Skip/開發中/待實作 列表）
    # 從 task_file 推導專案名稱
    project_name = task_file.parent.parent.name
    if project_name not in SKIP_DIRS:
        migrate_script = SCRIPTS_DIR / "migrate_readme.py"
        try:
            proc = subprocess.run(
                [sys.executable, str(migrate_script), project_name],
                capture_output=True, text=True, timeout=30
            )
            if proc.returncode == 0:
                result["readme_updated"] = True
            else:
                result["readme_error"] = proc.stderr.strip() or proc.stdout.strip()
...
def main():
    ...
    if args.json:
        ...
        if result.get("readme_updated"):
            print(f"  README.md 已同步")
        elif result.get("readme_error"):
            print(f"  ⚠️  README 更新失敗: {result['readme_error']}")
