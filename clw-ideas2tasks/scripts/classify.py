...
# ── 專案上下文（全域快取）──────────────────────────────────────
_projects_context_cache = None


def get_projects_context() -> str:
    """讀取 PROJECTS.md 並提取專案列表作為 LLM 上下文。"""
    global _projects_context_cache

    if _projects_context_cache is not None:
        return _projects_context_cache

    try:
        tasks_dir = os.environ.get("IDEAS2TASKS_TASKS_DIR", os.path.expanduser("~/Tasks"))
        projects_md = Path(tasks_dir) / "PROJECTS.md"

        if not projects_md.exists():
            _projects_context_cache = ""
            return ""
        ...
        for line in content.splitlines():
            line = line.strip()
            if line.startswith("|") and "專案" in line and "狀態" in line:
                in_table = True
                continue
            if in_table and line.startswith("|") and not line.startswith("|---"):
                ...
                if len(parts) >= 4:
                    link_part = parts[2]
                    m = re.search(r'\[([^\]]+)\]', link_part)
                    if m:
                        project_name = m.group(1)
                        if project_name and not project_name.startswith("http"):
                            projects.append(project_name)
            elif in_table and not line.startswith("|"):
                break

        if projects:
            context = "現有專案列表：" + ", ".join(projects[:20])
            _projects_context_cache = context
            return context
    except Exception:
        pass
...
def clear_projects_context_cache():
    """清除專案上下文快取（測試用）。"""
    global _projects_context_cache
    _projects_context_cache = None
...
def detect_category(text: str, context: str = "") -> str:
    """根據關鍵字偵測 category，回傳最可能的主類別。

    Args:
        text: idea 內容
        context: PROJECTS.md 提取的專案列表上下文（用於匹配現有專案）
    """
...
def extract_project_name_from_context(content: str, context: str) -> str:
    """從上下文中提取可能匹配的專案名稱。

    Args:
        content: idea 內容
        context: PROJECTS.md 專案列表
    ...
    projects_str = context.replace("現有專案列表：", "")
...
def classify_idea(idea: dict, use_context: bool = True) -> dict:
    """對單一 idea 進行完整分類分析。

    Args:
        idea: 包含 content 和 filename 的字典
        use_context: 是否使用 PROJECTS.md 上下文（預設 True）
    """
    content = idea["content"]
    filename = idea["filename"]

    context = get_projects_context() if use_context else ""
    matched_project = extract_project_name_from_context(content, context)

    done_tasks, pending_tasks = parse_done_markers(content)
    category = detect_category(content, context)
    ...
    project_name = matched_project if matched_project else Path(filename).stem.replace("_", "-")
