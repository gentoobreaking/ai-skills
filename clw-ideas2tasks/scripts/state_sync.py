...
try:
    ...
    IDEAS_DIR = _env_dir("IDEAS2TASKS_IDEAS_DIR", "Ideas")
except EnvironmentError as e:
    print(e, file=sys.stderr)
    sys.exit(1)


def read_frontmatter_field(content: str, field_name: str) -> str:
    """從 frontmatter 中讀取指定欄位，支援 markdown 標記與不分大小寫。"""
    lines = content.splitlines()
    for i, line in enumerate(lines):
        m = re.match(rf'^\-?[ \t]*(\*+[ \t]*)?{field_name}([ \t]*\*+)?[ \t]*:[ \t]*(.*)', line, re.IGNORECASE)
        if m:
            val = m.group(3).strip()
            if val:
                return val.strip('"').strip("'").split('#')[0].strip()
            # 向下相容：值可能在下一行（舊版 \s 跨行匹配遺留的損壞格式）
            if i + 1 < len(lines) and re.match(r'^\s*https?://\S+', lines[i + 1]):
...
def read_task_status(task_file: Path) -> str:
    ...
    try:
        ...
        raw_lower = read_frontmatter_field(content, "status").lower()
...
def read_task_blocked_by(task_file: Path) -> list[str]:
    """讀取 task 檔的 blocked_by 欄位，回傳阻擋任務 ID 列表。"""
    if not task_file.exists():
        return []
    try:
        content = task_file.read_text(encoding="utf-8")
        blocked = read_frontmatter_field(content, "blocked_by")
        if not blocked:
            return []
        blocked = blocked.strip()
        if blocked.startswith("[") and blocked.endswith("]"):
            items = blocked[1:-1].split(",")
...
def get_tasks_dir_status(project_name: str) -> dict:
    """
    掃描 $IDEAS2TASKS_TASKS_DIR/{project}/tasks/，回傳每個 task 的狀態。
...
def clean_duplicate_status(task_file: Path) -> bool:
    """
    清理 task 檔中多餘的 Status 行，只保留 frontmatter 的 status: 作為唯一來源。
    移除 body 中所有 Status: / - **Status**: 行。
    狀態值由 frontmatter 的 status: 欄位決定，不會遺失。
    """
    if not task_file.exists():
        return False
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if i <= fm_end:
                new_lines.append(line)
                continue
            # body 中匹配 Status 行（各種寫法）
            stripped = line.strip()
            if re.match(r'^(-?\s*(\*+\s*)?(\[\s*\])?\s*)?(\[?\s*)?Status(\s*\*+)?\s*:\s*.+', stripped, re.IGNORECASE):
                changed = True
                continue
            new_lines.append(line)
        if changed:
...
def normalize_all_task_statuses(project_name: str = None):
    ...
    清理：移除 body 中多餘的 Status 行（只保留 frontmatter 的 status:）。
    """
    if project_name:
    ...
    for proj_dir in dirs:
        ...
        for f in tasks_dir.glob("T*.md"):
            # 1. 清理 body 中多餘的 Status 行
            if clean_duplicate_status(f):
                fixed += 1
            # 2. 統一 frontmatter status 格式
...
def read_task_title(task_file: Path) -> str:
    """讀取 task 檔的標題。
    支援兩種格式：
    ...
    try:
        ...
        for line in content.splitlines():
            ...
            if stripped == "---":
                ...
                continue
            if in_frontmatter:
                title = read_frontmatter_field(content, "title")
                if title:
                    return title
...
def scan_tasks_dir() -> list[dict]:
    """
    掃描 $IDEAS2TASKS_TASKS_DIR/ 下所有專案，
