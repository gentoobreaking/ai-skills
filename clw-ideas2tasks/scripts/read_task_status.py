...
def read_all_task_status(project_dir: Path) -> dict:
    ...
    Args:
        project_dir: 專案目錄（由 IDEAS2TASKS_TASKS_DIR 決定）
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # 測試：讀取所有 Tasks 目錄
        import os as _os
        _val = _os.environ.get("IDEAS2TASKS_TASKS_DIR")
        if not _val:
            print("請設定環境變數 IDEAS2TASKS_TASKS_DIR")
            sys.exit(1)
        tasks_root = Path(_os.path.expanduser(_val)).resolve()
