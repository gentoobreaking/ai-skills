...
# Telegram 變數改為 send_telegram() 內即時讀取（不設則呼叫時拋錯）
...
def send_telegram(text: str) -> bool:
    ...
    if not bot_token or not chat_id:
        print("請設定環境變數 TELEGRAM_BOT_TOKEN 和 TELEGRAM_CHAT_ID", file=sys.stderr)
        sys.exit(1)
...
def run_scan(ideas_dir: str | None = None) -> list:
...
def main():
    ...
    parser.add_argument("--ideas-dir", default=None, help="自訂 Ideas 目錄（預設使用配置）")
    ...
    # 補上預設目錄（未指定 --ideas-dir 時）
    if args.ideas_dir is None:
        from scan import _IDEAS_DIR
        args.ideas_dir = str(_IDEAS_DIR)
